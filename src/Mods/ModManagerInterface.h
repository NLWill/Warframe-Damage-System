...
// Add mod at specified mod slot index
	virtual void AddMod(shared_ptr<Mod> mod, unsigned int modSlotIndex) = 0;
	// Add the mod at the first empty mod slot index that fits all requirements
	virtual void AddMod(shared_ptr<Mod> mod) = 0;
	...
	virtual shared_ptr<Mod> GetMod(unsigned int modSlotIndex) = 0;
