...
class Target{
	public:
	Target();
	Target(int level, float maxHealth, float maxArmour, Faction faction, HealthType healthType, std::map<std::string, std::pair<float, bool>> bodyPartMultipliers, std::vector<std::shared_ptr<Mod>> innateModUpgrades = {});
	Target(Target &other);
	...
	std::vector<std::shared_ptr<Mod>> innateUpgrades;
	...
	std::vector<ProcType> immuneStatusEffects;
